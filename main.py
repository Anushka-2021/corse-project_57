...
import requests, base64, sqlite3
...
(des_token UNIQUE, lang, code, input, exp_output, output)
""")
conn.commit()
...
def index():
    if request.method == 'POST':
        ...
        if rets is None:
            ...
            rs = rts.decode("UTF-8")
            cursor.execute("INSERT OR REPLACE INTO table57(des_token, lang, code, input, output) VALUES (?, ?, ?, ?, ?)", (decision_tok_n,myCodeLanguage, Cd, myCin, rs))
            conn.commit()
            noun = 0
            cursor.execute("SELECT * FROM table57")
            f = cursor.fetchone()
            while f:
                noun+=1
                f = cursor.fetchone()
